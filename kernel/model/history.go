...
func generateFileHistory() {
	...
	for _, box := range Conf.GetBoxes() {
		historyDir = filepath.Join(util.DataDir, box.ID, ".siyuan", "history") // 这个不要改为 .sillot
		os.RemoveAll(historyDir)
	}
	historyDir = filepath.Join(util.DataDir, "assets", ".siyuan", "history") // 这个不要改为 .sillot
	os.RemoveAll(historyDir)
	historyDir = filepath.Join(util.DataDir, ".siyuan", "history") // 这个不要改为 .sillot
	os.RemoveAll(historyDir)
}
...
func ClearWorkspaceHistory() (err error) {
	...
	for _, notebook := range notebooks {
		boxID := notebook.ID
		historyDir := filepath.Join(util.DataDir, boxID, ".siyuan", "history") // 这个不要改为 .sillot
		if !gulu.File.IsDir(historyDir) {
			continue
		}
