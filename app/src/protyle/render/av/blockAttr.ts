    ...
    switch (value.type) {
        ...
        case "date":
            if (value[value.type].isNotEmpty) {
                html = `<span data-content="${value[value.type].content}">${format(new Date(value[value.type].content), value[value.type].isNotTime ? "yyyy-MM-dd" : "yyyy-MM-dd HH:mm")}</span>`;
            }
            if (value[value.type].hasEndDate && value[value.type].isNotEmpty2 && value[value.type].isNotEmpty) {
                html += `<svg class="custom-attr__avarrow"><use xlink:href="#iconForward"></use></svg><span data-content="${value[value.type].content2}">${format(new Date(value[value.type].content2), value[value.type].isNotTime ? "yyyy-MM-dd" : "yyyy-MM-dd HH:mm")}</span>`;
            }
            break;
        case "created":
        case "updated":
