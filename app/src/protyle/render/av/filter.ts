...
<option ${"Is not empty" === options.filter.operator ? "selected" : ""} value="Is not empty">${window.siyuan.languages.filterOperatorIsNotEmpty}</option>`;
    ...
    <span class="fn__ellipsis">${option.name}</span>
        ...
        menu.addItem({
            iconHTML: "",
            label: `<input style="margin: 4px 0" value="${(dateValue.isNotEmpty || colData.type !== "date") ? format(new Date(dateValue.content), "yyyy-MM-dd") : ""}" type="date" max="9999-12-31" class="b3-text-field fn__size200">`
        });
        menu.addItem({
            iconHTML: "",
            label: `<input style="margin: 4px 0" value="${dateValue.isNotEmpty2 ? format(new Date(dateValue.content2), "yyyy-MM-dd") : ""}" type="date" max="9999-12-31" class="b3-text-field fn__size200">`
        });
    }
    menu.addItem({
        icon: "iconTrashcan",
            ...
            if (item.id === filter.column) {
                ...
                if (filter.operator === "Is empty") {
                    ...
                    if (filter.value?.date?.content2 && filter.operator === "Is between") {
                        filterValue = ` ${window.siyuan.languages.filterOperatorIsBetween} ${format(new Date(filter.value.date.content), "yyyy-MM-dd")} ${format(new Date(filter.value.date.content2), "yyyy-MM-dd")}`;
                    } else if ("=" === filter.operator) {
                        filterValue = `: ${format(new Date(filter.value.date.content), "yyyy-MM-dd")}`;
                    } else if ([">", "<"].includes(filter.operator)) {
                        filterValue = ` ${filter.operator} ${format(new Date(filter.value.date.content), "yyyy-MM-dd")}`;
                    } else if (">=" === filter.operator) {
                        filterValue = ` ≥ ${format(new Date(filter.value.date.content), "yyyy-MM-dd")}`;
                    } else if ("<=" === filter.operator) {
                        filterValue = ` ≤ ${format(new Date(filter.value.date.content), "yyyy-MM-dd")}`;
