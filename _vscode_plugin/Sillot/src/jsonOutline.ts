...
export class JsonOutlineProvider implements vscode.TreeDataProvider<number> {
    ...
    getChildren(offset?: number): Thenable<number[]> {
        if (offset && this.tree) {
            ...
            if (node) {return Promise.resolve(this.getChildrenOffsets(node));}
        }
    ...
    private getLabel(node: json.Node): string {
        if (node.parent?.type === "array") {
            ...
            const value = this.editor?.document.getText(
                new vscode.Range(this.editor.document.positionAt(node.offset), this.editor.document.positionAt(node.offset + node.length))
            );
